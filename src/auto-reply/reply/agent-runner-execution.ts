...
export async function runAgentTurnWithFallback(params: {
  ...
  const queueWaitStartedAt = params.followupRun.enqueuedAt;
  if (Number.isFinite(queueWaitStartedAt) && queueWaitStartedAt > 0) {
    const queueWaitSpan = startDiagnosticSpan(
      "queue.wait",
      {
        ...(params.sessionKey ? { session_key: params.sessionKey } : {}),
        heartbeat: params.isHeartbeat,
      },
      { startTimeMs: queueWaitStartedAt },
    );
    endDiagnosticSpan(queueWaitSpan, { status: "ok" });
  }
  return await withDiagnosticSpan(
    ...
    async () => {
      ...
      updateCurrentDiagnosticTraceMetadata({ sessionKey: params.sessionKey, runId });
