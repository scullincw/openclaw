...
import { runWithDiagnosticTraceKey, withDiagnosticSpan } from "../../infra/diagnostic-trace.js";
import {
  logMessageProcessed,
  logMessageQueued,
...
export async function dispatchReplyFromConfig(params: {
  ...
  const traceKey = (params.ctx as { DiagnosticTraceKey?: string }).DiagnosticTraceKey;
  return await runWithDiagnosticTraceKey(traceKey, async () => {
    ...
    const sessionStoreEntry = await withDiagnosticSpan(
      "session.acquire",
      {
        channel,
        ...(sessionKey ? { session_key: sessionKey } : {}),
      },
      async () => resolveSessionStoreLookup(ctx, cfg),
    );
